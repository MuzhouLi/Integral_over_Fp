...
s=int(sys.argv[4])#only traverse leftmost s branches, since one can add (n-s) rounds in the head.
...
for round_index in range(R):
	...
	r_i=round_index%n
	if r_i<s:
		WList[round_index]=xList[r_i]+var("k_"+str(round_index))
	else:
		WList[round_index]=var("k_"+str(round_index))
	ZList_BeforeS[round_index]=var("W_"+str(round_index))+sum_inList
	
	thisz=var("Z_"+str(round_index))
	
	for i in range(n-1):
		outList[i]=inList[i+1]
	outList[n-1]=thisz+inList[0]
...
#find the min Z index among these n outputs, and set TargetZ t be this Z index
...
print("Detecting Z_"+str(min_z)+" in branch "+str(min_i))
print()

TargetZ=min_z

file_name="gmimc_crf_"+str(p)+"_"+str(d)+"_"+str(n)+"_"+str(s)+"_"+str(R)+"_"+str(TargetZ)
...
z_index=[]
u_var_arr=[]
for round_index in range(TargetZ+1):
	
	tmpstr=str(ZList_BeforeS[round_index]).replace(" ","").split("+")[1:]
	#print(tmpstr)
	...
	z_index.append(this_z_index)
	
	max_v=(len(this_z_index)+1)*(d**(TargetZ+1-round_index))
	var_len=len_compute( math.log(max_v,2) )
	
	if var_len<math.ceil( math.log(p-1,2) ):
		var_len=math.ceil( math.log(p-1,2) )
	
	var_len_arr.append(var_len)
	
	f.write("v_"+str(round_index)+" : BITVECTOR("+str(var_len)+"); \n")
	
	this_u_var=[]
	for ele in tmpstr:
		x=int(ele.split("_")[1])
		
		f.write("u_"+str(round_index)+"_"+str(x)+" : BITVECTOR("+str(var_len)+"); \n")
		this_u_var.append(x)
	
	u_var_arr.append(this_u_var)
	
	f.write("this_right_"+str(round_index)+" : BITVECTOR("+str(var_len)+"); \n")

f.write("\n")

#basic conditions I: relations
for round_index in range(TargetZ,-1,-1):
	if len(z_index[round_index])>0:
		this_left=", v_"+str(round_index)
		f.write("ASSERT( BVGE( this_right_"+str(round_index)+", v_"+str(round_index)+") ); \n")
		for x in z_index[round_index]:
			this_left+=(", u_"+str(round_index)+"_"+str(x))
			f.write("ASSERT( BVGE( this_right_"+str(round_index)+", u_"+str(round_index)+"_"+str(x)+") ); \n")
		f.write("ASSERT( BVPLUS("+str( var_len_arr[round_index] )+this_left+") = this_right_"+str(round_index)+" ); \n")
	else:
		f.write("ASSERT( this_right_"+str(round_index)+" = v_"+str(round_index)+" ); \n")
	
	if round_index==TargetZ:
		this_right=int2binstr(d,var_len_arr[round_index])
	else:
		tmp_this_right=""
		for i in range(round_index+1,TargetZ+1):
			if round_index in z_index[i]:
				len_diff=var_len_arr[round_index]-var_len_arr[i]
				if len_diff>0:
					zero_add="0b"
					...
					zero_add=""
				tmp_this_right+=(", "+zero_add+"u_"+str(i)+"_"+str(round_index))
				f.write("ASSERT( BVGE( this_right_"+str(round_index)+" , "+zero_add+"u_"+str(i)+"_"+str(round_index)+" ) );\n")
				
		this_right=""
		for j in range(d):
			this_right+=tmp_this_right
		
		this_right="BVPLUS("+str(var_len_arr[round_index])+this_right+")"
		
	f.write("ASSERT( this_right_"+str(round_index)+" = "+this_right+" ); \n")
	
	f.write("\n")

#basic conditions II: combinations not zero mod p using Lucas Theorem
for round_index in range(TargetZ,0,-1):
	max_v=(d**(TargetZ+1-round_index))
	if max_v>p-1:
		max_p=math.floor(math.log(max_v,p))
		cur_len=len_compute( math.log(p**(max_p+1),2) )
		
		if cur_len<var_len_arr[round_index]:
			cur_len=var_len_arr[round_index]
		
		each_len=len_compute( math.log(p-1,2) )
		for comb_part in range(max_p+1):
			f.write("this_right_"+str(round_index)+"_p_"+str(comb_part)+" : BITVECTOR("+str(each_len)+"); \n")
			f.write("ASSERT( BVLE( this_right_"+str(round_index)+"_p_"+str(comb_part)+", "+int2binstr(p-1,each_len)+") ); \n")
			
			f.write("vp_"+str(round_index)+"_p_"+str(comb_part)+" : BITVECTOR("+str(each_len)+"); \n")
			f.write("ASSERT( BVLE( vp_"+str(round_index)+"_p_"+str(comb_part)+", "+int2binstr(p-1,each_len)+") ); \n")
			for x in z_index[round_index]:
				f.write("up_"+str(round_index)+"_"+str(x)+"_p_"+str(comb_part)+" : BITVECTOR("+str(each_len)+"); \n")
				f.write("ASSERT( BVLE( up_"+str(round_index)+"_"+str(x)+"_p_"+str(comb_part)+", "+int2binstr(p-1,each_len)+") ); \n")
		...
		len_diff=cur_len-var_len_arr[round_index]
		if len_diff>0:
			zero_add_2="0b"
			for j in range(len_diff):
				zero_add_2+="0"
		...
		for comb_part in range(max_p+1):
			if comb_part==0:
				right_hand+=(", "+zero_add+"this_right_"+str(round_index)+"_p_"+str(comb_part)+"")
				f.write("ASSERT( BVGE( "+zero_add_2+"this_right_"+str(round_index)+", "+zero_add+"this_right_"+str(round_index)+"_p_"+str(comb_part)+" ) ); \n")
			else:					
				right_hand+=(", BVMULT("+str(cur_len)+", "+zero_add+"this_right_"+str(round_index)+"_p_"+str(comb_part)+", "+int2binstr(p**comb_part,cur_len)+" )")
				f.write("ASSERT( BVGE( "+zero_add_2+"this_right_"+str(round_index)+", BVMULT("+str(cur_len)+", "+zero_add+"this_right_"+str(round_index)+"_p_"+str(comb_part)+", "+int2binstr(p**comb_part,cur_len)+" ) ) ); \n")
		
		if max_p>=1:
			f.write("ASSERT( "+zero_add_2+"this_right_"+str(round_index)+" = BVPLUS("+str(cur_len)+right_hand+") ); \n")
		else:
			f.write("ASSERT( "+zero_add_2+"this_right_"+str(round_index)+" = "+(right_hand[2:])+" ); \n")
		
		right_hand=""
		for comb_part in range(max_p+1):
			if comb_part==0:
				right_hand+=(", "+zero_add+"vp_"+str(round_index)+"_p_"+str(comb_part)+"")
				f.write("ASSERT( BVGE( "+zero_add_2+"v_"+str(round_index)+", "+zero_add+"vp_"+str(round_index)+"_p_"+str(comb_part)+" ) ); \n")
			else:					
				right_hand+=(", BVMULT("+str(cur_len)+", "+zero_add+"vp_"+str(round_index)+"_p_"+str(comb_part)+", "+int2binstr(p**comb_part,cur_len)+" )")
				f.write("ASSERT( BVGE( "+zero_add_2+"v_"+str(round_index)+", BVMULT("+str(cur_len)+", "+zero_add+"vp_"+str(round_index)+"_p_"+str(comb_part)+", "+int2binstr(p**comb_part,cur_len)+" ) ) ); \n")
		
		if max_p>=1:
			f.write("ASSERT( "+zero_add_2+"v_"+str(round_index)+" = BVPLUS("+str(cur_len)+right_hand+") ); \n")
		else:
			f.write("ASSERT( "+zero_add_2+"v_"+str(round_index)+" = "+(right_hand[2:])+" ); \n")
		
		for x in z_index[round_index]:			
			right_hand=""
			for comb_part in range(max_p+1):
				if comb_part==0:
					right_hand+=(", "+zero_add+"up_"+str(round_index)+"_"+str(x)+"_p_"+str(comb_part)+"")
					f.write("ASSERT( BVGE( "+zero_add_2+"u_"+str(round_index)+"_"+str(x)+", "+zero_add+"up_"+str(round_index)+"_"+str(x)+"_p_"+str(comb_part)+" ) ); \n")
				else:
					right_hand+=(", BVMULT("+str(cur_len)+", "+zero_add+"up_"+str(round_index)+"_"+str(x)+"_p_"+str(comb_part)+", "+int2binstr(p**comb_part,cur_len)+" )")
					f.write("ASSERT( BVGE( "+zero_add_2+"u_"+str(round_index)+"_"+str(x)+", BVMULT("+str(cur_len)+", "+zero_add+"up_"+str(round_index)+"_"+str(x)+"_p_"+str(comb_part)+", "+int2binstr(p**comb_part,cur_len)+" ) ) ); \n")
			
			if max_p>=1:
				f.write("ASSERT( "+zero_add_2+"u_"+str(round_index)+"_"+str(x)+" = BVPLUS("+str(cur_len)+right_hand+") ); \n")
			else:
				f.write("ASSERT( "+zero_add_2+"u_"+str(round_index)+"_"+str(x)+" = "+(right_hand[2:])+" ); \n")
		
		#print(round_index,z_index[round_index],len(z_index[round_index]))
		sum_each_len=len_compute( math.log((len(z_index[round_index])+1)*(p-1),2) )
		...
		for comb_part in range(max_p+1):
			right_hand=", "+zero_add+"vp_"+str(round_index)+"_p_"+str(comb_part)+""
			f.write("ASSERT( BVGE( this_right_"+str(round_index)+"_p_"+str(comb_part)+", vp_"+str(round_index)+"_p_"+str(comb_part)+") ); \n")
			for x in z_index[round_index]:
				right_hand+=(", "+zero_add+"up_"+str(round_index)+"_"+str(x)+"_p_"+str(comb_part)+"")
				f.write("ASSERT( BVGE( this_right_"+str(round_index)+"_p_"+str(comb_part)+", up_"+str(round_index)+"_"+str(x)+"_p_"+str(comb_part)+") ); \n")
			if max_p>=1:
				f.write("ASSERT( "+zero_add+"this_right_"+str(round_index)+"_p_"+str(comb_part)+" = BVPLUS("+str(sum_each_len)+right_hand+") ); \n")
			else:
				f.write("ASSERT( "+zero_add+"this_right_"+str(round_index)+"_p_"+str(comb_part)+" = "+(right_hand[2:])+" ); \n")
	f.write("\n")
f.write("\n")

#target vars and conditions
for round_index in range(TargetZ+1):
	f.write("Fv_"+str(round_index)+" : BITVECTOR("+str(var_len_arr[round_index])+"); \n")
	
	max_len=len_compute( math.log(p-1,2) )
	zero_add=""
	if max_len<=var_len_arr[round_index]:
		max_len=var_len_arr[round_index]
	else:
		len_diff=max_len-var_len_arr[round_index]
		zero_add="0b"
		for j in range(len_diff):
			zero_add+="0"
		zero_add+="@"
	
	f.write("ASSERT( IF ((BVGE("+zero_add+"v_"+str(round_index)+","+int2binstr(p-1,max_len)+")) AND (BVMOD("+str(max_len)+","+zero_add+"v_"+str(round_index)+","+int2binstr(p-1,max_len)+")="+int2binstr(0,max_len)+")) THEN "+zero_add+"Fv_"+str(round_index)+"="+int2binstr(p-1,max_len)+" ELSE "+zero_add+"Fv_"+str(round_index)+"=BVMOD("+str(max_len)+","+zero_add+"v_"+str(round_index)+","+int2binstr(p-1,max_len)+") ENDIF ); \n")

f.write("\n")

condition_Fv=[[] for w in range(s)]
for round_index in range(TargetZ+1):
	this_w=round_index%n
	if this_w<=s-1:
		condition_Fv[this_w].append(round_index)
...
def Special_Comb_D1(round_index, m1_var, m1_len, m2_var, m2_len, Fv1_var, Fv1_var_len, Fv0_var, Fv0_var_len):
	max_tmp=m1_len
	if m2_len>max_tmp:
		max_tmp=m2_len
	...
	f.write("cond_var_"+str(round_index)+" : BITVECTOR("+str(cond_var_len)+"); \n")
	...
	f.write("ASSERT( BVLE( cond_var_"+str(round_index)+" , "+right_eq+" ) ); \n")
	...
	left_eq=("BVPLUS("+str(cond_var_len)+" , cond_var_"+str(round_index)+" , "+left_eq+" )")
	
	f.write("ASSERT( "+left_eq+" = "+right_eq+" ); \n")
	f.write("ASSERT( BVMOD( "+str(cond_var_len)+" , cond_var_"+str(round_index)+" , "+int2binstr(p-1,cond_var_len)+" ) = "+int2binstr(0,cond_var_len)+" ); \n")
	f.write("\n")
	...
	f.write(m1_var+"_label_"+str(round_index)+"_1, "+m1_var+"_label_"+str(round_index)+"_2, "+m1_var+"_label_"+str(round_index)+"_3, "+m1_var+"_label_"+str(round_index)+"_4 : BITVECTOR(1); \n")
	
	conds=["( "+m1_var+"_label_"+str(round_index)+"_1 = 0b1 )", "( "+m1_var+"_label_"+str(round_index)+"_2 = 0b1 )", "( "+m1_var+"_label_"+str(round_index)+"_3 = 0b1 )", "( "+m1_var+"_label_"+str(round_index)+"_4 = 0b1 )"]
	
	f.write("ASSERT( (("+Fv0_var+" = "+int2binstr(p-1,Fv0_var_len)+") AND ("+Fv1_var+" = "+int2binstr(p-1,Fv1_var_len)+") AND (BVGE("+m2_var+","+int2binstr(p-1,m2_len)+")) ) => ("+m1_var+"_label_"+str(round_index)+"_1 = 0b1) ); \n")
	f.write("ASSERT( (("+Fv0_var+" = "+int2binstr(p-1,Fv0_var_len)+") AND ("+Fv1_var+" = "+int2binstr(p-1,Fv1_var_len)+") AND (BVLT("+m2_var+","+int2binstr(p-1,m2_len)+")) ) => ("+m1_var+"_label_"+str(round_index)+"_2 = 0b1) ); \n")
	
	f.write("ASSERT( (("+Fv0_var+" = "+int2binstr(p-1,Fv0_var_len)+") AND ( BVGE( "+Fv1_var+" , "+int2binstr(1,Fv1_var_len)+" ) ) AND ( BVLE( "+Fv1_var+" , "+int2binstr(p-2,Fv1_var_len)+" ) ) AND (BVGE("+m2_var+","+int2binstr(p-1,m2_len)+")) ) => ("+m1_var+"_label_"+str(round_index)+"_3 = 0b1) ); \n")
	f.write("ASSERT( (("+Fv0_var+" = "+int2binstr(p-1,Fv0_var_len)+") AND ( BVGE( "+Fv1_var+" , "+int2binstr(1,Fv1_var_len)+" ) ) AND ( BVLE( "+Fv1_var+" , "+int2binstr(p-2,Fv1_var_len)+" ) ) AND (BVLT("+m2_var+","+int2binstr(p-1,m2_len)+")) ) => ("+m1_var+"_label_"+str(round_index)+"_4 = 0b1) ); \n")
	
	f.write("ASSERT( (( BVGE( "+Fv0_var+" , "+int2binstr(1,Fv0_var_len)+" ) ) AND ( BVLE( "+Fv0_var+" , "+int2binstr(p-2,Fv0_var_len)+" ) ) AND ("+Fv1_var+" = "+int2binstr(p-1,Fv1_var_len)+") AND (BVGE( "+zero_add+m2_var+" , "+m1_var+"_Fv0_plus_pminus1 )) ) => ("+m1_var+"_label_"+str(round_index)+"_1 = 0b1) ); \n")
	f.write("ASSERT( (( BVGE( "+Fv0_var+" , "+int2binstr(1,Fv0_var_len)+" ) ) AND ( BVLE( "+Fv0_var+" , "+int2binstr(p-2,Fv0_var_len)+" ) ) AND ("+Fv1_var+" = "+int2binstr(p-1,Fv1_var_len)+") AND (BVLT( "+zero_add+m2_var+" , "+m1_var+"_Fv0_plus_pminus1 )) ) => ("+m1_var+"_label_"+str(round_index)+"_2 = 0b1) ); \n")
	
	f.write("ASSERT( (( BVGE( "+Fv0_var+" , "+int2binstr(1,Fv0_var_len)+" ) ) AND ( BVLE( "+Fv0_var+" , "+int2binstr(p-2,Fv0_var_len)+" ) ) AND ( BVGE( "+Fv1_var+" , "+int2binstr(1,Fv1_var_len)+" ) ) AND ( BVLE( "+Fv1_var+" , "+int2binstr(p-2,Fv1_var_len)+" ) ) AND (BVGE( "+zero_add+m2_var+" , "+m1_var+"_Fv0_plus_pminus1 )) ) => ("+m1_var+"_label_"+str(round_index)+"_3 = 0b1) ); \n")
	f.write("ASSERT( (( BVGE( "+Fv0_var+" , "+int2binstr(1,Fv0_var_len)+" ) ) AND ( BVLE( "+Fv0_var+" , "+int2binstr(p-2,Fv0_var_len)+" ) ) AND ( BVGE( "+Fv1_var+" , "+int2binstr(1,Fv1_var_len)+" ) ) AND ( BVLE( "+Fv1_var+" , "+int2binstr(p-2,Fv1_var_len)+" ) ) AND (BVLT( "+zero_add+m2_var+" , "+m1_var+"_Fv0_plus_pminus1 )) ) => ("+m1_var+"_label_"+str(round_index)+"_4 = 0b1) ); \n")
...
for round_index in range(1,TargetZ+1):
	max_v=(d**(TargetZ+1-round_index))
	if max_v>p-1:
		#construct the input of Special_Comb_D1
		if 0 in z_index[round_index]:
			m1_minused_part=[]
			for ele in z_index[round_index]:
				if ele!=0:
					m1_minused_part.append(ele)
			
			m2_part=[]
			for tmp_round in range(1,TargetZ+1):
				if tmp_round!=round_index and 0 in z_index[tmp_round]:
					m2_part.append(tmp_round)
			
			this_Comb=["MultiComb( m1, [v_"+str(round_index)+" , u_"+str(round_index)+"_0])","v_"+str(round_index)+" + u_"+str(round_index)+"_0 = m1", "v0 = d*u_"+str(round_index)+"_0 + m2","Fv_"+str(round_index),"Fv_0"]
			
			tmpstr=""
			for ele in m1_minused_part:
				tmpstr+=(" + u_"+str(round_index)+"_"+str(ele))
			tmpstr=tmpstr[3:]
			if tmpstr=="":
				tmpstr="m1 = TR_"+str(round_index)
			else:
				tmpstr="m1 = TR_"+str(round_index)+" - ("+tmpstr+")"
			this_Comb.append(tmpstr)
			
			tmpstr=""
			for ele in m2_part:
				tmpstr+=(" + u_"+str(ele)+"_0")
			tmpstr=tmpstr[3:]
			if len(m2_part)>=1:
				tmpstr=("m2 = d*("+tmpstr+")")
			else:
			...
			if not Check_Repeat(all_need_considered_combs,this_Comb):
				...
				m1_var="m1_r_"+str(round_index)
				m1_len=var_len_arr[round_index]
				...
				f.write("ASSERT( this_right_"+str(round_index)+" = "+tmpstr+" ); \n")
				f.write("ASSERT( BVLE( "+m1_var+" , this_right_"+str(round_index)+" ) ); \n")
				
				m2_var="m2_r_"+str(round_index)
				m2_len=var_len_arr[round_index]+len_compute( math.log(d,2) )
				...
				for ele in m2_part:
					...
					if len_diff>0:
						...
						zero_add2=""
					for j in range(d):
						tmpstr+=(" , "+zero_add+"u_"+str(ele)+"_0")
					f.write("ASSERT( BVGE( "+zero_add2+m2_var+" , "+zero_add+"u_"+str(ele)+"_0 ) ); \n")
				if len(m2_part)>=1:
					f.write("ASSERT( "+zero_add2+m2_var+" = BVPLUS( "+str(max_len_here)+tmpstr+" ) ); \n")
				else:
					f.write("ASSERT( "+m2_var+" = "+int2binstr(0,m2_len)+" ); \n")
				
				Fv1_var="Fv_"+str(round_index)
				Fv1_var_len=var_len_arr[round_index]
				
				Fv0_var="Fv_0"
				Fv0_var_len=var_len_arr[0]
				
				Special_Comb_D1(round_index, m1_var, m1_len, m2_var, m2_len, Fv1_var, Fv1_var_len, Fv0_var, Fv0_var_len)

def List_All_Size2(index_same_part,all_need_considered_combs,round_index_1,round_index_2,fix_v):
	if 0 in index_same_part:#form is [v,u_*_0,u_*_a]
		#fixed u_*_a vars
		this_Comb=["MultiComb( m1, [v_"+str(round_index_2)+" , u_"+str(round_index_2)+"_0]) * MultiComb( m2, [v_"+str(round_index_1)+" , u_"+str(round_index_1)+"_0])","v_"+str(round_index_2)+" + u_"+str(round_index_2)+"_0 = m1","v_"+str(round_index_1)+" + u_"+str(round_index_1)+"_0 = m2","v_0 = d*(u_"+str(round_index_2)+"_0 + u_"+str(round_index_1)+"_0) + m3","Fv_"+str(round_index_2),"Fv_"+str(round_index_1),"Fv_0"]
		
		tmpstr=""
		for ele in u_var_arr[round_index_2]:
			if ele!=0:
				tmpstr+=(" + u_"+str(round_index_2)+"_"+str(ele))
		tmpstr=tmpstr[3:]
		if tmpstr=="":
			tmpstr="m1 = TR_"+str(round_index_2)
		else:
			tmpstr="m1 = TR_"+str(round_index_2)+" - ("+tmpstr+")"
		this_Comb.append(tmpstr)
		
		tmpstr=""
		for ele in u_var_arr[round_index_1]:
			if ele!=0:
				tmpstr+=(" + u_"+str(round_index_1)+"_"+str(ele))
		tmpstr=tmpstr[3:]
		if tmpstr=="":
			tmpstr="m2 = TR_"+str(round_index_1)
		else:
			tmpstr="m2 = TR_"+str(round_index_1)+" - ("+tmpstr+")"
		this_Comb.append(tmpstr)
		
		m3_part=[]
		for tmp_round in range(1,TargetZ+1):
			if tmp_round!=round_index_1 and tmp_round!=round_index_2 and 0 in z_index[tmp_round]:
				m3_part.append(tmp_round)
		tmpstr=""
		for ele in m3_part:
			tmpstr+=(" + u_"+str(ele)+"_0")
		tmpstr=tmpstr[3:]
		...
		if not Check_Repeat(all_need_considered_combs,this_Comb):
			...
			extra_label_index="_2v0"
			for i in index_same_part:
				extra_label_index+=("_"+str(i))
			
			m1_var="m1_rr_"+str(round_index_1)+"_"+str(round_index_2)+extra_label_index
			m1_len=var_len_arr[round_index_1]#var_len_arr[round_index_2] <= var_len_arr[round_index_1]
			f.write(m1_var+" : BITVECTOR("+str(m1_len)+"); \n")
			
			len_diff=m1_len-var_len_arr[round_index_2]
			if len_diff>0:
				zero_add="0b"
				for j in range(len_diff):
					zero_add+="0"
			...
			for ele in u_var_arr[round_index_2]:
				if ele != 0:
					tmpstr+=(" , "+zero_add+"u_"+str(round_index_2)+"_"+str(ele))
			if tmpstr=="":
				tmpstr=m1_var
			else:
				tmpstr=("BVPLUS( "+str(m1_len)+", "+m1_var+tmpstr+" )")
			f.write("ASSERT( "+zero_add+"this_right_"+str(round_index_2)+" = "+tmpstr+" ); \n")
			f.write("ASSERT( BVLE( "+m1_var+" , "+zero_add+"this_right_"+str(round_index_2)+" ) ); \n")
			
			m2_var="m2_rr_"+str(round_index_1)+"_"+str(round_index_2)+extra_label_index
			m2_len=var_len_arr[round_index_1]#var_len_arr[round_index_2] <= var_len_arr[round_index_1]
			f.write(m2_var+" : BITVECTOR("+str(m2_len)+"); \n")
			...
			f.write("ASSERT( this_right_"+str(round_index_1)+" = "+tmpstr+" ); \n")
			f.write("ASSERT( BVLE( "+m2_var+" , this_right_"+str(round_index_1)+" ) ); \n")
			
			m3_var="m3_rr_"+str(round_index_1)+"_"+str(round_index_2)+extra_label_index
			m3_len=var_len_arr[0]+len_compute( math.log(d,2) )
			...
			for ele in m3_part:
				...
				for j in range(d):
					tmpstr+=(" , "+zero_add+"u_"+str(ele)+"_0")
				f.write("ASSERT( BVGE( "+zero_add2+m3_var+" , "+zero_add+"u_"+str(ele)+"_0 ) ); \n")
			if len(m3_part)>=1:
				f.write("ASSERT( "+zero_add2+m3_var+" = BVPLUS( "+str(max_len_here)+tmpstr+" ) ); \n")
			else:
				f.write("ASSERT( "+m3_var+" = "+int2binstr(0,m3_len)+" ); \n")
			
			Fv2_var="Fv_"+str(round_index_2)
			Fv2_var_len=var_len_arr[round_index_2]
			
			Fv1_var="Fv_"+str(round_index_1)
			Fv1_var_len=var_len_arr[round_index_1]
			
			Fv0_var="Fv_0"
			Fv0_var_len=var_len_arr[0]
			
			Special_Comb_D2(round_index_1,round_index_2, m1_var, m1_len, m2_var, m2_len, m3_var, m3_len, Fv2_var, Fv2_var_len, Fv1_var, Fv1_var_len, Fv0_var, Fv0_var_len,extra_label_index)
			
		#fixed v_vars
		if fix_v:
			this_Comb=["MultiComb( m1 + m2, [uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(index_same_part[1])+" , m1 + m2 - uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(index_same_part[1])+"])"]
			
			tmpstr=""
			for ele in u_var_arr[round_index_2]:
				if ele not in index_same_part:
					tmpstr+=(" + u_"+str(round_index_2)+"_"+str(ele))
			tmpstr+=(" + v_"+str(round_index_2))
			tmpstr=tmpstr[3:]
			if tmpstr=="":
				tmpstr="m1 = TR_"+str(round_index_2)
			else:
			...
			tmpstr=("uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(index_same_part[1])+" = u_"+str(round_index_2)+"_"+str(index_same_part[1])+" + u_"+str(round_index_1)+"_"+str(index_same_part[1]))
			this_Comb.append(tmpstr)
			...
			if not Check_Repeat(all_need_considered_combs,this_Comb):
				...
				for ele in u_var_arr[round_index_2]:
					if ele not in index_same_part:
						tmpstr+=(" , "+zero_add+"u_"+str(round_index_2)+"_"+str(ele))
				tmpstr+=(" , "+zero_add+"v_"+str(round_index_2))
				if tmpstr=="":
					tmpstr=m1_var
				else:
					tmpstr=("BVPLUS( "+str(m1_len)+", "+m1_var+tmpstr+" )")
				...
				m1plusm2_var="m1plusm2_"+str(round_index_1)+"_"+str(round_index_2)+extra_label_index
				m1plusm2_len=1+var_len_arr[round_index_1]
				f.write("m1plusm2_"+str(round_index_1)+"_"+str(round_index_2)+extra_label_index+" : BITVECTOR("+str(m1plusm2_len)+"); \n")
				...
				last_var="last_var_"+str(round_index_1)+"_"+str(round_index_2)+extra_label_index
				last_var_len=1+var_len_arr[round_index_1]
				f.write("last_var_"+str(round_index_1)+"_"+str(round_index_2)+extra_label_index+" : BITVECTOR("+str(last_var_len)+"); \n")
				
				uhat_var_arr=[]
				for ele in index_same_part:
					if ele!=0:
						uhat_var_arr.append(("uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)))
				uhat_var_len=1+var_len_arr[round_index_1]
		...
		#fixed v_vars
		this_Comb=["MultiComb( m1 + m2, [ uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(index_same_part[0])+" , uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(index_same_part[1])+"])"]
		...
		for ele in index_same_part:
			tmpstr=("uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+" = u_"+str(round_index_2)+"_"+str(ele)+" + u_"+str(round_index_1)+"_"+str(ele))
			this_Comb.append(tmpstr)
			
			tmpstr="uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+" >= m1"#cond_1
			this_Comb.append(tmpstr)
			
			tmpstr="uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+" >= m2"#cond_2
			this_Comb.append(tmpstr)
...
def List_All(index_same_part,all_need_considered_combs,round_index_1,round_index_2):
	if len(index_same_part)>=2:
		if len(index_same_part)==2:
			List_All_Size2(index_same_part,all_need_considered_combs,round_index_1,round_index_2,True)
		else:
			if 0 in index_same_part:
				#'''
				#fixed one of these u_*_j vars where j>=1
				for ele in index_same_part:
					if ele!=0:
						tmp_index_same_part=[0,ele]
						print(tmp_index_same_part)
						List_All_Size2(tmp_index_same_part,all_need_considered_combs,round_index_1,round_index_2,False)
				#'''
				#fixed v_vars
				this_Comb=[]
				...
				for ele in index_same_part:
					if ele!=0:
						this_tmp_str="uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)
				...
				List_All(tmp_index_same_part,all_need_considered_combs,round_index_1,round_index_2)
				...
				for ele in index_same_part:
					if ele!=0:
						...
						tmpstr="uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+" >= m1"
						this_Comb.append(tmpstr)
						
						tmpstr="uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+" >= m2"
						this_Comb.append(tmpstr)
...
for round_index_1 in range(1,TargetZ+1):
	max_v_1=(d**(TargetZ+1-round_index_1))
	if max_v_1>p-1:
		for round_index_2 in range(round_index_1+1,TargetZ+1):
			max_v_2=(d**(TargetZ+1-round_index_2))
			if max_v_2>p-1:
				all_possible=[]
				
				index_same_part=[]
				for ele in u_var_arr[round_index_1]:
					if ele in u_var_arr[round_index_2]:
						index_same_part.append(ele)
				
				print(round_index_1,u_var_arr[round_index_1])
				print(round_index_2,u_var_arr[round_index_2])
				...
				for ele in index_same_part:
					if ele!=0:
						f.write("uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+" : BITVECTOR("+str((1+var_len_arr[round_index_1]))+"); \n")
						
						len_diff=1+var_len_arr[round_index_1]-var_len_arr[round_index_2]
						if len_diff>0:
							zero_add="0b"
							for j in range(len_diff):
								zero_add+="0"
							...
							zero_add=""
						f.write("ASSERT( uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+" = BVPLUS("+str((1+var_len_arr[round_index_1]))+", 0b0@u_"+str(round_index_1)+"_"+str(ele)+", "+zero_add+"u_"+str(round_index_2)+"_"+str(ele)+" ) ); \n")
						f.write("ASSERT( BVGE( uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+" , 0b0@u_"+str(round_index_1)+"_"+str(ele)+" ) ); \n")
						f.write("ASSERT( BVGE( uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+" , "+zero_add+"u_"+str(round_index_2)+"_"+str(ele)+" ) ); \n")
						f.write("\n")
						...
						if cur_len<var_len_arr[round_index_1]+1:
							cur_len=var_len_arr[round_index_1]+1
						
						each_len=len_compute( math.log(p-1,2) )
						for comb_part in range(max_p+1):
							f.write("uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+"_p_"+str(comb_part)+" : BITVECTOR("+str(each_len)+"); \n")
							f.write("ASSERT( BVLE( uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+"_p_"+str(comb_part)+", "+int2binstr(p-1,each_len)+") ); \n")
						...
						len_diff=cur_len-var_len_arr[round_index_1]-1
						if len_diff>0:
							zero_add_2="0b"
							for j in range(len_diff):
								zero_add_2+="0"
						...
						for comb_part in range(max_p+1):
							if comb_part==0:
								right_hand+=(", "+zero_add+"uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+"_p_"+str(comb_part)+"")
								f.write("ASSERT( BVGE( "+zero_add_2+"uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+", "+zero_add+"uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+"_p_"+str(comb_part)+" ) ); \n")
							else:					
								right_hand+=(", BVMULT("+str(cur_len)+", "+zero_add+"uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+"_p_"+str(comb_part)+", "+int2binstr(p**comb_part,cur_len)+" )")
								f.write("ASSERT( BVGE( "+zero_add_2+"uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+", BVMULT("+str(cur_len)+", "+zero_add+"uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+"_p_"+str(comb_part)+", "+int2binstr(p**comb_part,cur_len)+" ) ) ); \n")
						if max_p>=1:
							f.write("ASSERT( "+zero_add_2+"uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+" = BVPLUS("+str(cur_len)+right_hand+") ); \n")
						else:
							f.write("ASSERT( "+zero_add_2+"uhat_"+str(round_index_1)+"_"+str(round_index_2)+"_"+str(ele)+" = "+(right_hand[2:])+" ); \n")
						f.write("\n")
				
				if len(index_same_part)>0:
					if len(index_same_part)==1:
						if 0 in index_same_part:#form is [v,u_*_0]
							this_Comb=["MultiComb( m1, [v_"+str(round_index_2)+" , u_"+str(round_index_2)+"_0]) * MultiComb( m2, [v_"+str(round_index_1)+" , u_"+str(round_index_1)+"_0])","v_"+str(round_index_2)+" + u_"+str(round_index_2)+"_0 = m1","v_"+str(round_index_1)+" + u_"+str(round_index_1)+"_0 = m2","v_0 = d*(u_"+str(round_index_2)+"_0 + u_"+str(round_index_1)+"_0) + m3","Fv_"+str(round_index_2),"Fv_"+str(round_index_1),"Fv_0"]
						...
						List_All(index_same_part,all_need_considered_combs,round_index_1,round_index_2)
