...
rot_number_pos=[]
max_num_L=math.ceil( R/2.0 )
mod_num=math.ceil( math.log(n/2.0,2) )
for num_L in range(max_num_L):
	rot_number_pos.append( 0 )
	rot_number_pos.append( 2**(num_L%mod_num) )
print("rot_num : ",rot_number_pos)
print()
...
for round_index in range(R):
	...
	for branch in range(group_num):
		...
		for i in range( n ):
			if "x_"+str(i)+"_" in str(inList[branch]):
				tmpV=inList[branch]-xList[i]
				WList[W_index] = xList[i]+var("k_"+str(W_index))
				ZList_BeforeS[W_index] = var("W_"+str(W_index))+tmpV
			else:
				if "c_"+str(i) in str(inList[branch]):
					tmpV=inList[branch]-xList[i]
					WList[W_index] = var("k_"+str(W_index))
					ZList_BeforeS[W_index] = var("W_"+str(W_index))+tmpV
	
	for branch in range(group_num):
		outList[group_num+branch] = inList[branch]
	
	for branch in range(group_num):
		after_Z_index = group_num*round_index+(branch-rot_number_pos[round_index]+group_num)%group_num
		thisz=var("Z_"+str(after_Z_index))
		outList[branch] = inList[group_num+branch] + thisz
...
for target_z_index in tmpTargetZ:
	...
	file_name="gmimc_mrf_"+str(p)+"_"+str(d)+"_"+str(n)+"_"+str(s)+"_"+str(R)+"_"+str(TargetZ)+"_pos"+str_pos
